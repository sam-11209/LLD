...
/**
 * Represents a physical locker site (hub or station) containing sets of lockers categorized by size.
 * Coordinates package placement using an assignment strategy.
 */
public class Site {
    ...
    /**
     * Constructs a Site with a specific ID, name, locker capacity configurations, and locker assignment strategy.
     *
     * @param siteId             the unique site identifier
     * @param siteName           the human-readable name of the site
     * @param lockerCounts       map indicating the number of lockers to instantiate for each size
     * @param assignmentStrategy the LockerAssignmentStrategy strategy (defaults to SmallestFit if null)
     */
    public Site(String siteId, String siteName, Map<LockerSize, Integer> lockerCounts, LockerAssignmentStrategy assignmentStrategy) {
        this.siteId = siteId;
        this.siteName = siteName;
        this.assignmentStrategy = assignmentStrategy != null ? assignmentStrategy : new SmallestFitLockerAssignmentStrategy();
    ...
    /**
     * Convenience constructor creating a default Site with ID "SITE-1" and name "Main Site".
     *
    ...
    /**
     * Delegate method to find an available locker using the configured assignment strategy.
     *
    ...
    /**
     * Places a package into a fitting, available locker thread-safely for a given deposit date.
     * Searches starting from the package's minimum required size up to larger sizes.
     *
     ...
     * @throws NoLockerAvailableException if no suitable locker is currently available
     */
    public Locker placePackage(ShippingPackage pkg, Date date) {
        ...
        throw new NoLockerAvailableException("No suitable locker available for package orderId: " + pkg.getOrderId() + " requiring size: " + requiredSize);
    }
}
