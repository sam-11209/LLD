...
public class FirstComeFirstServe implements DispatchingStrategy {
    ...
    /**
     * Selects the first elevator that is:
     *  1. IDLE (best case — it's free)
     *  2. Already moving in the same direction AND on the correct side
     *     (so it can pick up the passenger on the way).
     *
    ...
    public ElevatorCar selectElevator(List<ElevatorCar> elevators,
        ...
        for (ElevatorCar car : elevators) {
            ...
            if (carDir == dir) {
                if (dir == Direction.UP   && carFloor <= floor) return car;
                if (dir == Direction.DOWN && carFloor >= floor) return car;
            }
